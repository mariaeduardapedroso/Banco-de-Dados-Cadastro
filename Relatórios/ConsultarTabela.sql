...
CREATE PROCEDURE TABELAS
@TABELA AS VARCHAR(20)
AS
BEGIN 

	IF @TABELA = 'ASSINATURAS'
		SELECT * FROM [dbo].[Assinaturas]

	IF @TABELA = 'CADASTROSTRANSACOES'
		SELECT * FROM [dbo].[CadastrosTransacoes]

	IF @TABELA = 'CADASTROSUSUARIO'
		SELECT * FROM [dbo].[CadastrosUsuario]

	IF @TABELA = 'CLASSIFICACOES'
		SELECT * FROM [dbo].[Classificacoes]

	IF @TABELA = 'CREDENCIAIS'
		SELECT * FROM [dbo].[Credenciais]

	IF @TABELA = 'ENTRADAS'
		SELECT * FROM [dbo].[Entradas]
	...
	ELSE
		PRINT 'ERRO NA ESCOLHA DA TABELA, TABELA INEXISTENTE'
END
----------------------------------------------------------------------------------------------------------
...
SET @TABELA_NOME = 'MUDE O NOME AQUI'

EXEC TABELAS @TABELA_NOME 
